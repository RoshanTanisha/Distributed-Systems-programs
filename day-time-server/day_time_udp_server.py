...
class DayTimeUDPServer(UDPServer):
    ...
    def _handle_current_connection(self, client_address_string):
        ...
        if 'time' in data_from_client.lower():
            message_from_server = 'From server: {}, data is: {}'.format(self.convert_address_to_string((self.ip, self.port)), time.strftime('%d/%m/%y %H:%M:%S'))
...
if __name__ == "__main__":
    server = DayTimeUDPServer('127.0.0.1', 5001)
    try:
